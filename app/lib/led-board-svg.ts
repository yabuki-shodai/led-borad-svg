...
const dotStartX = 8;
const dotStartY = 10;
const defaultText = "電光掲示板の演出   SVG TREE   README DISPLAY   ";
...
function renderDots(kind: "off" | "on") {
  const radius = kind === "off" ? "2.35" : "3";
  const attrs =
    kind === "off"
      ? 'fill="#3d444c" opacity="0.7"'
      : 'fill="url(#led-light)"';
...
export function createLedBoardSvg(options: LedBoardSvgOptions = {}) {
  ...
  <defs>
    ...
    <linearGradient id="led-light" x1="0" x2="1" y1="0" y2="0">
      <stop offset="0%" stop-color="#ffb3f4" />
  ...
  <rect width="760" height="112" fill="#05070a" />
  <rect x="1" y="1" width="758" height="110" rx="2" fill="url(#led-board-bg)" />
  <g>${renderDots("off")}</g>
  <g clip-path="url(#led-window)">
    <text x="780" y="66" fill="#ff9df2" font-family="Hiragino Kaku Gothic ProN, Yu Gothic, Meiryo, ui-sans-serif, system-ui, sans-serif" font-size="58" font-weight="800" letter-spacing="2" dominant-baseline="middle" filter="url(#led-text-haze)" opacity="0.42">
      ${text}
      <animate attributeName="x" from="780" to="-1560" dur="${duration}s" repeatCount="indefinite" />
    </text>
    <text x="784" y="66" fill="#7dd3fc" font-family="Hiragino Kaku Gothic ProN, Yu Gothic, Meiryo, ui-sans-serif, system-ui, sans-serif" font-size="58" font-weight="800" letter-spacing="2" dominant-baseline="middle" filter="url(#led-text-haze)" opacity="0.36">
      ${text}
      <animate attributeName="x" from="784" to="-1556" dur="${duration}s" repeatCount="indefinite" />
    </text>
  </g>
  <g mask="url(#led-text-mask)" filter="url(#led-soft-glow)">${renderDots("on")}</g>
  <rect x="1" y="1" width="758" height="110" rx="2" fill="none" stroke="#2f3741" />
</svg>`;
}
